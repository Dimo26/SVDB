#!/usr/bin/env python3
"""
Workaround: Extract SVs from BAM, save as VCF, then build database

This bypasses the Cython compilation issue by:
1. Using readBAM.py to extract SVs
2. Writing them to a VCF file
3. Building database from the VCF
"""
...
def variants_to_vcf(variants, output_vcf, sample_name):
    """Convert extracted variants to VCF format"""
    ...
    with open(output_vcf, 'w') as f:
        ...
        for i, (chrA, posA, chrB, posB, sv_type, INFO, FORMAT) in enumerate(variants, 1):
            ...
            if sv_type == "DEL":
                ...
                # Translocation - use breakend notation
                alt = f"N[{chrB}:{posB}["
            else:
                alt = f"<{sv_type}>"
...
def main():
    ...
    if not variants:
        print("✗ No variants extracted from BAM file")
        return 1
    ...
    # Step 2: Write to VCF
    print(f"[2/3] Converting to VCF format")
    ...
    # Step 3: Build database from VCF
    print(f"[3/3] Building SVDB database")
